...
public class ProductDAOImpl implements ProductDAO {
    ...
    public ArrayList<Product> getProductList(int start,
	...
	try {
	    ...
	    ps = conn
		    .prepareStatement("SELECT * FROM product_tb WHERE product_name LIKE '%"
			    + keyWord + "%' limit ?,?");
	    ps.setInt(1, start);
	    ps.setInt(2, limit);
	...
	try {
	    conn = DBConfig.getConnection();
	    ps = conn
		    .prepareStatement("select count(*) AS RowsCount from product_tb where product_name like '%"
			    + keyWord + "%' ");
	    rs = ps.executeQuery();
	    while (rs.next()) {
	...
	try {
	    ...
	    ps = conn
		    .prepareStatement("INSERT INTO `product_tb` " +
		    		"(control_item_id,product_name,specification," +
	    ...
	    ps.setString(5, p.getUse_restriction()) ;
	    ps.setString(6, p.getManufacturer()) ;
	    ps.setString(7, p.getProduct_img()) ;
	    ps.executeUpdate() ;
	} catch (Exception e) {
	    e.printStackTrace();
	    return false;
	...
	try {
	    conn = DBConfig.getConnection();
	    ps = conn
		    .prepareStatement("UPDATE `product_tb` SET  control_item_id = ?,product_name = ?,specification = ? ,unit = ?" +
